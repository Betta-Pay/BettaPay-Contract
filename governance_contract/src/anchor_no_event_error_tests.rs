...
fn change_threshold_emits_no_event_when_insufficient_signatures() {
    // 2-of-2 admin set. After the N-of-N fix, change_threshold requires
    // current_threshold (2) signers — a single signer is insufficient.
    let env = Env::default();
    env.mock_all_auths();
    ...
    // Threshold is 2, so change_threshold requires 2 signatures — a single
    // signer is insufficient.
