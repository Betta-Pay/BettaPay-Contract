...
/// Re-registration removes the ArchivedMerchant tombstone (issue #685),
/// so the re-registered merchant can read old payment records that were
/// stored before unregister. The records survive because they are keyed
/// by (merchant, reference) and never deleted.
#[test]
fn reregistered_merchant_can_read_old_payment_records() {
    let (env, _gov_client, _gov_admins, settle_client, settle_admins, merchant) = setup_both();
    settle_client.register_merchant(&settle_admins, &merchant);
    ...
    // Tombstone was removed on re-register (issue #685), so old records
    // are readable again.
    let record = settle_client
        .get_payment_reference(&merchant, &reference)
        .expect("re-registered merchant must be able to read old records");
...
fn set_settlement_rule_emits_fallback_and_updated_events() {
    ...
    // (issue #691 removed bootstrap_fallback from the read path).
    let events = env.events().all();
